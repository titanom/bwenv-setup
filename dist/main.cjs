...
if (typeof __dirname2 === "undefined") {
  __dirname2 = path.resolve();
}
var __dirname2;
var configFileSchema = objectType({
...
async function getVersionFromConfigFile() {
  ...
  if (!filepath)
    throw new Error("Could not find configuration file in root directory.");
  const raw = await fsp.readFile(filepath, "utf8");
  const parse2 = filepath.endsWith("toml") ? import_toml.parse : import_yaml.parse;
  const parsed = parse2(raw);
  const config = configFileSchema.parse(parsed);
  return config.version;
}
function findLatestMatchingVersion(versions, targetVersion) {
  const matchingVersions = versions.filter((version2) => semver.satisfies(version2, targetVersion));
...
async function getAvailableVersions() {
  ...
  const response = await http.getJson(releaseURL);
  if (response.result) {
...
function getBinaryType() {
  ...
  const arch2 = os.arch();
  core.info(`Platform: ${platform2}`);
  core.info(`Arch: ${arch2}`);
  let binary = "";
  switch (platform2) {
    case "darwin": {
      if (arch2 === "x64") {
...
async function getReleaseURL(version2 = "latest", binaryType) {
  const _version = version2 === "latest" ? await getLatestVersion() : `v${version2.replace("v", "")}`;
...
async function run() {
  try {
    ...
    const toolDir = await tc.cacheFile(path.join(__dirname2, "bwenv"), "bwenv", "bwenv", version2);
